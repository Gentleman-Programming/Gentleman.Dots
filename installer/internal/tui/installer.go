...
func stepInstallNvim(m *Model) error {
	...
	// Write custom leader key configuration
	if err := writeNvimLeaderConfig(m); err != nil {
		return wrapStepError("nvim", "Install Neovim",
			"Failed to write leader key configuration",
			err)
	}
...
// writeNvimLeaderConfig writes the custom leader key and experience level to options.lua
func writeNvimLeaderConfig(m *Model) error {
	homeDir := os.Getenv("HOME")
	optionsPath := filepath.Join(homeDir, ".config/nvim/lua/config/options.lua")

	leader := m.Choices.LeaderKey
	luaLeader := " " // Default space
	switch leader {
	case "comma":
		luaLeader = ","
	case "backslash":
	...
	// Read existing options.lua if it exists
	content := ""
	if data, err := os.ReadFile(optionsPath); err == nil {
		content = string(data)
	}

	// Prepare the leader config block
	var sb strings.Builder
	sb.WriteString("-- Leader key configuration (Generated by Gentleman.Dots)\n")
	sb.WriteString(fmt.Sprintf("vim.g.mapleader = \"%s\"\n", luaLeader))
	sb.WriteString(fmt.Sprintf("vim.g.maplocalleader = \"%s\"\n\n", luaLeader))
