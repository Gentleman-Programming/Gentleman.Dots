...
head_export() {
  ...
  git -C "$REPO" show "HEAD:sketchybar/sketchybarrc" | sed -n "s/^export ${variable}=//p"
}
...
assert_eq "$(head_export ISLAND_BG)" "$(resolve_value ISLAND_BG)" "missing marker defaults to Gentleman island background"
assert_eq "$(head_export WHITE)" "$(resolve_value TEXT)" "Gentleman text preserves the current white"
assert_eq "$(head_export DIM)" "$(resolve_value DIM)" "Gentleman dim preserves the current dim"
assert_eq "$(head_export ACCENT_COLOR)" "$(resolve_value ACCENT)" "Gentleman accent preserves the current accent"
assert_eq "$(head_export ISLAND_BORDER)" "$(resolve_value ISLAND_BORDER)" "Gentleman border preserves the current island border"
assert_eq "$(head_export ISLAND_BG)" "$(resolve_value SELECTED_BG)" "Gentleman selected workspace preserves the current island background"
assert_eq "$(head_export CYAN)" "$(resolve_value CPU_NORMAL)" "Gentleman normal CPU preserves the current cyan"
assert_eq "$(head_export RED)" "$(resolve_value CPU_INITIAL)" "Gentleman initial CPU preserves the old red"
assert_eq '0xffe0c15a' "$(resolve_value CHARGING)" "Gentleman charging preserves the existing accent"
assert_eq "$(head_export ORANGE)" "$(resolve_value GPU_NORMAL)" "Gentleman normal GPU preserves the current orange"
assert_eq "$(head_export MAGENTA)" "$(resolve_value RAM_NORMAL)" "Gentleman normal RAM preserves the current magenta"
assert_eq "$(head_export BLUE)" "$(resolve_value VOLUME_NORMAL)" "Gentleman normal volume preserves the current blue"
...
assert_eq "$(head_export ISLAND_BG)" "$(resolve_value ISLAND_BG)" "invalid marker defaults to Gentleman"

printf 'gentleman\n' > "$MARKER_DIR/sketchybar-theme"
assert_eq "$(head_export RED)" "$(resolve_value ERROR)" "Gentleman error preserves the current red"
assert_eq "$(head_export YELLOW)" "$(resolve_value WARNING)" "Gentleman warning preserves the current yellow"
...
assert_eq '0xffe35fa6' "$(resolve_value ACCENT)" "Cute accent"
assert_eq '0xffc15995' "$(resolve_value DEEP_ROSE)" "Cute deep rose"
assert_eq '0xffff81cc' "$(resolve_value ACTIVE_ROSE)" "Cute active rose"
...
assert_eq '0xffff81cc' "$(resolve_value WORKSPACE_ACTIVE)" "Cute selected workspace uses active rose"
...
assert_eq '0xffe35fa6' "$(resolve_value VOLUME_NORMAL)" "Cute normal volume uses primary rose"
assert_eq '0xffc15995' "$(resolve_value RAM_NORMAL)" "Cute RAM uses deep rose"
...
assert_contains "color=$(head_export TRANSPARENT)" "$main_output" "bar preserves its transparent base"
assert_contains "icon.color=$(head_export WHITE)" "$main_output" "default text preserves its current white"
assert_contains "label.color=$(head_export DIM)" "$main_output" "workspace labels preserve their dim color"
assert_contains "icon.color=$(head_export ACCENT_COLOR)" "$main_output" "separator preserves the current accent"
assert_contains "label.color=$(head_export BLACK)" "$main_output" "front app text preserves its current black"
assert_contains "background.color=$(head_export GREEN)" "$main_output" "front app preserves its current green island"
assert_contains "icon.color=$(head_export BLUE)" "$main_output" "volume preserves its initial blue"
assert_eq "$(head_export RED)" "$(icon_color_for_item cpu "$main_output")" "Gentleman initial CPU renders the old red"
assert_contains "icon.color=$(head_export ORANGE)" "$main_output" "GPU preserves its initial orange"
assert_contains "icon.color=$(head_export MAGENTA)" "$main_output" "RAM preserves its initial magenta"
...
assert_contains 'label.color=0xffff81cc' "$cute_space_selected" "Cute selected workspace uses active rose"
assert_contains 'background.color=0xff342230' "$cute_space_selected" "Cute selected workspace uses selected background"
assert_contains 'background.border_color=0xffff81cc' "$cute_space_selected" "Cute selected workspace uses active rose border"
