{ config, pkgs, ... }:
let
  # Determinamos el sistema: si es macOS (darwin) o Linux
  systemType = if builtins.substring 0 6 (toString config.system) == "darwin" then "mac" else "linux";

  # Definimos el contenido inline de config.nu y env.nu (usá el contenido que me pasaste)
  nushellConfigText = ''
let dark_theme = {
    separator: "#C9C7CD"
